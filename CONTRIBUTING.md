...
[个人扩展](docs/extensions.md) 和内置 `development` Skill，不要把个人代码写入 `mochi/skills/`。
开发默认开启，无需单独授权；Main 使用 `inspect_extension`、
`write_extension`、`run_extension`、`activate_extension` 自主完成开发与实时激活，
无需 Admin 或重启。下文的源码目录与重启步骤只用于内置 Skill 开发。
...
工具定义格式：`## Tools` → `### tool_name (load)` → 参数表：
...
`load` 只能是 `resident`、`routed` 或 `on_demand`。它描述工具何时进入
当前轮次，不代表抽象风险等级。状态变化、可恢复性、回执、拒绝规则和限流应在
具体工具契约与执行代码中明确表达。
...
Observer 是被动的只读传感器，为 Heartbeat Main 处境提供上下文 — 零 LLM 调用。

两种类型：

| 类型 | 位置 | 开关 | 示例 |
|------|------|------|------|
| **共置型** | `mochi/skills/{name}/` | 跟随 skill 开关 | weather, reminder |
...
| 记忆提取、召回 | 心跳主动消息发送 |
| DB 读写、schema 迁移 | Observer 真实外部 API 调用 |
| Admin portal API | |
