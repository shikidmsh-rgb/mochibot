...
## Unreleased
- **主动消息不再隔天补发**：Free Time 和 Attention 发送失败后结束当次投递，不再跨轮或重启后重播旧稿。
- **提醒最多延迟 5 分钟**：普通定时提醒和 Self Reminder 只在预定时间后的 5 分钟内重试；过期后不再启动处理或发送，避免下午补发昨晚的睡觉提醒。
- **微信主动消息整条发送**：保留段落和换行，只有超过长度上限才拆分，减少发送额度消耗和连续通知；普通聊天仍保留分气泡。
- **微信回复凭据支持重启恢复**：保存并恢复当前账号与收件人对应的最近回复凭据，减少重启后无法发送消息的问题。
- **发送失败原因更清晰**：区分发送条件未就绪、接口明确拒绝和送达结果不确定，并记录错误码，便于排查。

升级后，遗留的主动消息不会补发，超过有效期的提醒会自动作废；原始记录和已经完成的操作保留。微信平台的回复额度限制仍然存在。
