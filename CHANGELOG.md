...
### 修复
- **完成回报路由到 todo**：用户说"猫砂盆清理完了 / 地板拖完了"等汇报完成的消息，pre-router 之前会判成纯聊天或别的技能，导致 Chat LLM 手里没 `manage_todo` 工具、无法标记对应待办为完成。在 todo SKILL.md 的 description 末尾补一句"用户回报完成某事且不属于活跃习惯时也走这里"，与 router prompt 已有的"当前活跃习惯"提示形成二选一锚点。E2E 实测（gpt-5.2-chat lite，每条 3 次）：完成回报类从 0/3 → 3/3 命中 todo，已注册的 habit（运动/吃药/喝水）仍稳定走 habit 不被误抢，整体 5/8 → 8/8。
