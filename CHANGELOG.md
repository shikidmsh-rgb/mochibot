...
- **微信收文件**：Mochi 能读懂发来的文本、PDF、Word 等文件。
- **追问更连贯**：对话中用过的工具会延续，追问不再"失忆"。
- **回复更快更省**：大幅提高缓存命中。
- **支持 GPT-6 Sol**。
- **修复**：重启偶尔误退出、部分接口工具调用报错。

升级提示：新增依赖，升级后请重新安装依赖。

## v1.0.18

- **微信收图**：可接收单张图片，由 Main 直接理解，原图不保存。
- **微信生图（可选）**：在后台「模型 → 图片生成」配置后，Mochi 可以在微信聊天中生成并发送图片。
- **修复 GPT-5.6 等模型调用工具时报错**。
- **修复回复里偶尔冒出时间戳**。

升级提示：收图需要 Main 模型支持看图；生图会产生服务商费用。
