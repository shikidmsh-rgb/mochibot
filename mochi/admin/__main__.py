...
def _admin_browser_url(host: str, port: int, token: str | None) -> str:
    url = f"http://{host}:{port}"
    return f"{url}?token={token}" if token else url


def _admin_log_url(host: str, port: int) -> str:
    """Return the credential-free URL that is safe to write to logs."""
...
def main():
    ...
    url = _admin_browser_url(browser_host, args.port, token)
    log.info("Admin portal: %s", _admin_log_url(browser_host, args.port))
