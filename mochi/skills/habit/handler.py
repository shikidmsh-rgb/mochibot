"""Habit skill handler — recurring habit tracking with check-in, pause/snooze, and stats.
...
class HabitSkill(Skill):
    ...
    def _checkin(self, user_id: int, args: dict) -> SkillResult:
        ...
        # Dedup guard: reject rapid duplicate calls (race condition in parallel tool calls)
        dedup_seconds = int(self.get_config("checkin_dedup_seconds") or 5)
        if existing and count == 1:
            last_checkin_str = existing[-1].get("logged_at", "")
            try:
                ...
                if seconds_since < dedup_seconds:
                    return SkillResult(
                        output=f"⚠️ {habit['name']} was just checked in {int(seconds_since)}s ago, ignoring duplicate.",
                    )
            except Exception:
                pass
    ...
    def _snooze(self, user_id: int, args: dict) -> SkillResult:
        snooze_default = int(self.get_config("snooze_default_min") or 60)
        habit_id = args.get("habit_id")
        if not habit_id:
            return SkillResult(output="Error: 'habit_id' is required for snooze.", success=False)
        delay = int(args.get("delay_minutes", 0) or snooze_default)
        if delay <= 0:
            delay = snooze_default
        ...
        ok = snooze_habit(user_id, int(habit_id), until_dt.isoformat())
        if not ok:
            return SkillResult(output=f"Habit #{habit_id} not found.", success=False)

        # Auto-create a reminder at snooze-end (cross-skill, guarded)
        reminder_msg = ""
        try:
            from mochi.db import create_reminder
            create_reminder(user_id, channel_id=0,
                            message=f"Time for {name}",
                            remind_at=until_dt.isoformat())
            reminder_msg = f", reminder set for {until_dt.strftime('%H:%M')}"
        except Exception:
            log.warning("Could not create snooze reminder (reminder system unavailable)")
            reminder_msg = ""

        return SkillResult(output=f"💤 {name} snoozed for {delay}min{reminder_msg}")
