...
def _uses_english_search(query: str) -> bool:
    return bool(_ASCII_LETTER_RE.search(query)) and all(
        not char.isalpha() or char.isascii() for char in query
    )
...
class _BingSearchParser(HTMLParser):
    """Extract organic result cards without executing page content."""
...
def _extract_bing_results(
    body: bytes, encoding: str | None, max_results: int,
) -> str:
    try:
    ...
    if not parser.results:
        raise ValueError("Bing search returned no web results.")
    return "\n\n".join(
        f"{index}. {title}\n   {href}\n   {snippet}"
...
async def _bing_search_within_deadline(query: str, max_results: int) -> str:
    ...
    async with httpx.AsyncClient(
        ...
        async with client.stream(
            "GET", _BING_SEARCH_URL, params=params,
        ) as response:
    ...
    return await asyncio.to_thread(
        _extract_bing_results, body, encoding, max_results,
    )
...
def _format_api_results(
    ...
    for item in raw_references:
        if not isinstance(item, dict) or item.get("type", "web") != "web":
            continue
        title = _single_line([
            str(item.get("title") or item.get("web_anchor") or "")
        ])
...
async def _web_search(
    ...
    try:
        return await _baidu_search(
            query, api_key=api_key, max_results=max_results, recency=recency,
        )
        ...
        return (
            ...
            + (" and the requested recency filter was not enforced" if recency else "")
            + ".]\n\n"
            f"{fallback}"
        )
