...
class WebSearchSkill(Skill):
    async def execute(self, context: SkillContext) -> SkillResult:
        ...
        timeout_s = int(self.get_config("WEB_SEARCH_TIMEOUT_S") or 20)
        default_max = int(self.get_config("WEB_SEARCH_MAX_RESULTS") or 5)
        max_results = context.args.get("max_results", default_max)
        max_results = max(1, min(10, int(max_results)))

        # Update cache params from config
        cache_ttl = int(self.get_config("WEB_SEARCH_CACHE_TTL_S") or 300)
        cache_size = int(self.get_config("WEB_SEARCH_CACHE_SIZE") or 256)
        _cache._ttl_s = cache_ttl
        _cache._max_size = cache_size

        try:
            result = await _ddg_search(query, max_results=max_results, timeout_s=timeout_s)
