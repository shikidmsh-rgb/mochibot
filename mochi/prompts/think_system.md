...
- **Notes** — 用户用自然语言写的监控/陪伴请求。每条自己判断"现在该不该触发"
  - 例："监控我脂肪摄入" → 看 meal observer 当天数据
  - 例："上班时容易情绪失控提醒我" → 看时间 + 对话调性
  - 例："没事给我发个新闻" → 看是不是真的没事
  - 例："我说在开会就闭嘴一小时" → 看用户最近一句话和时间，决定**不出** finding
...
- `summary`: 给 chat 看的事实陈述。**写事实和判断依据，不要写"该跟用户说什么话"**——表达是 chat 的事
...
按用途分两类——**事件型**（该报就报）vs **氛围型**（看场合）：

**事件型**（不受沉默时长影响，该报就报）：
- `habit_nudge` — habit 逾期/连续打卡断了
- `todo_reminder` — todo 截止临近/错过
- `reminder_due` — 显式提醒到点
- `note_triggered` — notes 里某条监控条件命中
- `morning_briefing` — 早晨第一拍，议程概览
- `maintenance_alert` — 系统/设备异常

**氛围型**（看场合，刚说完话就别吐，避免回弹感）：
- `idle_presence` — 用户沉默+氛围允许，让 chat 看着办要不要冒泡
- `vibe_check` — 对话余韵需要跟进
