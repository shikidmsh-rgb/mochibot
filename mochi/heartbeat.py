...
async def _act(result: dict, user_id: int) -> None:
    """Execute the Think decision. Supports both old and new formats.

    Old: {"type": "notify", "content": "..."} — single action
    New: {"actions": [...], "thought": "..."} — array of actions
    """
    ...
    if not actions and result.get("type"):
        actions = [result]  # old format: single action dict
    ...
    for action in actions:
        ...
        if action_type == "notify":
            ...
            # Support both formats: direct content or structured summary
            content = action.get("content") or action.get("summary", "")
            ...
            if content and _send_callback:
                ...
                log_heartbeat(_state, f"notify:{topic}", content[:100])
                log.info("Proactive message sent [%s] (%d/%d today)",
                         topic, _proactive_count_today, _effective('MAX_DAILY_PROACTIVE'))
            ...
            # Write to diary journal section
            try:
                from mochi.diary import diary
...
async def heartbeat_loop() -> None:
    ...
    while True:
        try:
            ...
            if sleep_action:
                if _send_callback:
                    # Use Think LLM to generate natural goodnight message
                    hint = sleep_action["context_hint"]
                    silence_h = sleep_action["silence_hours"]
                    goodnight_obs = {
                        "type": "silence_sleep",
                        "context_hint": hint,
                        "silence_hours": silence_h,
                        "hour": hour,
                    }
                    goodnight_msg = await _llm_with_timeout(
                        _generate_goodnight(goodnight_obs, user_id), "goodnight")
                    if goodnight_msg:
...
async def _generate_goodnight(context: dict, user_id: int) -> str | None:
    """Generate a natural goodnight message via Think LLM.

    context: {"type": "silence_sleep", "context_hint": "first_sleep"|"re_sleep",
              "silence_hours": float, "hour": int}
    """
    prompt = get_prompt("think_system")
    if not prompt:
        return None
    ...
    obs_text = json.dumps({
        "task": "generate_goodnight",
        "context_hint": context["context_hint"],
        "silence_hours": context["silence_hours"],
        "hour": context["hour"],
        "instruction": (
            "The user has gone silent at night — they likely fell asleep. "
            "Generate a short, warm goodnight message (1-2 sentences). "
            'If context_hint is "re_sleep", they woke briefly and fell asleep again. '
    ...
    try:
        ...
        log_usage(
            response.prompt_tokens, response.completion_tokens,
            response.total_tokens, model=response.model, purpose="goodnight",
        )
        content = response.content.strip()
        return content if content else None
    except Exception as e:
        log.error("Goodnight generation failed: %s", e)
        return None
