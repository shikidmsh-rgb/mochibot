...
新个人工具使用 `mochi.mod_api.v1` 公开接口并声明 `mod_api: 1`；旧格式与旧导入无需迁移。
...
**加载方式**（追加在工具名后）：
- `resident` — 每个适用轮次都可用
- `routed` — 只出现在 Lite pre-router 的候选中，被选中后进入该轮
...
- add 会创建一条持久记录，并在结果中返回新 ID
- list 只读取现有记录，不改变状态
- delete 会移动记录到可恢复的回收站
```
