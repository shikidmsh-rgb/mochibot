...
> Telegram 和微信均支持文字和单张、5 MB 以内的图片，需要 Main 模型支持看图。微信图片支持 JPG、PNG、GIF、WebP，下载或解析失败会明确提示；原图仅用于当轮对话，不保存到聊天历史。Telegram 还支持表情包；语音音频暂未接入模型。

微信也支持发送图片：后台「高级 → 图片生成」可选择本地图片直接发给主人，不调用模型。
如需生成新图，在同一卡片填写生图模型、API Key 和 API 根地址。官方地址可以自动识别，
兼容服务需选择 OpenAI Images 或 Gemini 原生接口；模型名按服务商提供的填写。
保存不会收费；点击「试生成」才发送你填写的描述并可能计费，不带聊天记录或记忆。
试生成成功后仍需单独点击「发送此图到主人微信」。微信需要已登录且有主人的可用回复上下文；
若提示未就绪，先在微信给 Mochi 发一条消息。此处是手动基础能力，尚未增加 Main 的生图工具。
