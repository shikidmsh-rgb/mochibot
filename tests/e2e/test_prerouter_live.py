"""Live E2E test for pre-router — calls real LLM via mochitest's admin DB config.

Run:
    cd M:/mochitest
    .venv/Scripts/python -m pytest tests/e2e/test_prerouter_live.py -v -s

Uses mochitest's real DB (admin portal tier config) — NOT mock LLM.
"""
...
# ── Load mochitest .env BEFORE mochi imports ──
from dotenv import load_dotenv
load_dotenv(Path("M:/mochitest/.env"), override=True)

logging.basicConfig(level=logging.INFO, format="%(name)s | %(message)s")
log = logging.getLogger("test_prerouter_live")

# Force DB to mochitest's real database
MOCHITEST_DB = Path("M:/mochitest/data/mochi.db")
...
def fresh_db(monkeypatch):
    """Override conftest: use mochitest's real DB instead of tmp."""
    if not MOCHITEST_DB.exists():
        pytest.skip(f"mochitest DB not found: {MOCHITEST_DB}")
    import mochi.db as db_module
    monkeypatch.setattr(db_module, "DB_PATH", MOCHITEST_DB)
