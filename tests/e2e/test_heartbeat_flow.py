...
class TestSilenceSleepE2E:
    ...
    async def test_silence_sleep_sends_goodnight(self, monkeypatch, mock_llm_factory):
        """Heartbeat loop sends LLM-generated goodnight on silence sleep."""
        import mochi.heartbeat as hb
        ...
        # Mock LLM returns a goodnight message
        mock = mock_llm_factory([make_response("Looks like you fell asleep. Goodnight!")])
        # Also patch heartbeat's get_client
        monkeypatch.setattr(hb, "get_client", lambda *a, **kw: mock)

        # Run the silence sleep check + delivery (same logic as heartbeat_loop)
        sleep_action = hb.check_silence_sleep()
        assert sleep_action is not None

        context = sleep_action
        goodnight_msg = await hb._generate_goodnight(
