...
class TestGetOrCreateEntity:
    def test_create_new(self):
        eid = get_or_create_entity(UID, "红糖", entity_type="pet")
        assert isinstance(eid, int) and eid > 0

    def test_idempotent(self):
        eid1 = get_or_create_entity(UID, "红糖", entity_type="pet")
        eid2 = get_or_create_entity(UID, "红糖", entity_type="pet")
        assert eid1 == eid2
...
class TestGetEntityByName:
    def test_found(self):
        get_or_create_entity(UID, "红糖", entity_type="pet")
        entity = get_entity_by_name(UID, "红糖")
        assert entity is not None
        assert entity["name"] == "红糖"
...
class TestAddTriple:
    def test_basic(self):
        s = get_or_create_entity(UID, "红糖", entity_type="pet")
        o = get_or_create_entity(UID, "甲亢", entity_type="concept")
    ...
    def test_multi_valued_no_invalidation(self):
        s = get_or_create_entity(UID, "红糖", entity_type="pet")
        o1 = get_or_create_entity(UID, "甲亢", entity_type="concept")
...
class TestQueryEntity:
    ...
    def test_as_object(self):
        ...
        o = get_or_create_entity(UID, "红糖", entity_type="pet")
...
class TestFindMatchingEntities:
    def test_basic_match(self):
        get_or_create_entity(UID, "红糖", entity_type="pet")
        matched = find_matching_entities(UID, "今天红糖怎么样")
        assert "红糖" in matched
...
class TestEntityContextForPrompt:
    def test_basic_format(self):
        s = get_or_create_entity(UID, "红糖", entity_type="pet", display_name="红糖")
        o = get_or_create_entity(UID, "甲亢", entity_type="concept")
        add_triple(UID, s, "has_condition", o)

        text = entity_context_for_prompt(UID, "红糖")
        assert "红糖" in text
...
class TestExtractKg:
    def test_extract_with_mock_llm(self, monkeypatch):
        ...
        save_message(UID, "user", "我家猫红糖最近去看了医生，医生说她得了甲亢，需要吃药控制")
        save_message(UID, "assistant", "红糖快点好起来！甲亢是可以通过药物控制的")

        # Mock LLM response
        class MockResponse:
            content = '{"entities":[{"name":"红糖","type":"pet"}],"triples":[{"subject":"红糖","predicate":"has_condition","object":"甲亢"}]}'
            prompt_tokens = 100
