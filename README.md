...
- **自己开发小工具**——默认具备，无需单独授权；Main 自己读取规范、编写个人 Python Skill、运行调试并实时激活，无需去后台或手动重启。新工具通过正常的按需请求进入同一轮对话的后续模型调用。个人代码和数据不随官方更新被覆盖，后台可关闭开发能力。可信本地代码，不是沙箱；详见 [个人扩展](docs/extensions.md)。
...
| **personal_workspace** | 统一浏览和编辑个人资料、工具草稿；可独立运行、启用工具，关闭开发不影响资料 |
