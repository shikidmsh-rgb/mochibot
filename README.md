...
- **Core**——保存在 `data/core.md` 的一份自由文本，每次对话都在；从一句“刚刚醒来”开始，随着相处持续更新
...
- **个人工作区**——Main 从同一入口读写 Markdown 资料与工具草稿；自己决定何时创作或开发，不会自动分类、注入或代写
- **每周记忆整理**——Nightly 只可靠归档；每周由同一个 Main 回看真实证据，谨慎整理 Core 与记忆项

自动去重只忽略空白差异，不会因为文字或向量相似就丢弃改口、修正或补充；不同内容保留为独立记忆，不自动覆盖旧记录。相似度仍用于查找相关记忆。

普通聊天的自动召回默认没有时间冷却，连续消息也能检索各自相关的记忆；仍保留筛选和长度预算。启用 Embedding 时，新查询可能增加请求和等待；需要冷却时可显式设置 `MEMORY_AUTO_RECALL_COOLDOWN`（秒）。
...
- **联网搜索**——问当前事件、新闻、价格，可配置 Tavily 或百度千帆 API Key；无需 Key 也可使用 Bing。配置 Tavily 时使用 Tavily，失败会明确报错；否则沿用百度优先、Bing 备用，切换时注明
...
- **开发者扩展机制**——可向项目添加内置 Skill 和 Observer，无需修改主流程；源码开发方式见 [贡献指南](CONTRIBUTING.md)
- **自己开发小工具**——直接描述需求，Mochi 可以选择使用现有工具、保存资料，或开发可重复调用的**个人扩展**。开发默认开启，无需另行授权、去后台操作或手动重启；个人代码和数据不随官方更新被覆盖。后台可关闭开发能力。运行的是可信本地 Python，不是沙箱；详见 [个人扩展](docs/extensions.md)。
...
内置 Skill 可在管理后台管理。源码开发者在 `mochi/skills/` 下新增 Skill 后，重启即可注册，不需要改主流程代码。个人扩展使用独立工作区，不必修改官方源码或重启。
...
| **web_search** | 联网搜索——可在技能配置中设置 `TAVILY_API_KEY` 或 `BAIDU_API_KEY`；Tavily 优先，未配置时沿用百度/Bing |
| **sticker** | 表情包——学习你转发的贴纸，聊天时自动发（仅 Telegram） |
| **skill_management** | 技能管理——通过对话列出、开关、配置所有技能 |
| **personal_workspace** | 保存个人资料，开发、调试和启用个人扩展；关闭开发不影响资料功能 |

> **想向项目贡献内置 Skill？** 在 `mochi/skills/` 下新建文件夹，放入 `SKILL.md` + `handler.py`，重启 bot 就会自动注册。详见 [CONTRIBUTING.md](CONTRIBUTING.md)。

> **个人扩展不必改官方源码。** 可以把它理解成游戏里的 Mod：给现有程序增加能力。使用时只需描述需求，不必先了解这些技术名词。扩展保存在 `data/extensions/`，通过 `mochi.mod_api.v1` 和 `SKILL.md` 中的 `mod_api: 1` 使用稳定的公开接口；这些技术名称保持不变，旧格式与旧导入无需迁移。官方更新保留个人代码、配置和数据，Base 内部重构应保持 v1 行为兼容；任意第三方依赖和外部服务变化不在此保证内。目前支持工具扩展，不支持 Observer、共享数据库迁移或自动安装依赖，详见[个人扩展规范](docs/extensions.md)。
...
| `FREE_TIME_ENABLED` | `true` | Free Time 调度开关；仅从 `.env` 读取，重启生效 |
...
`FREE_TIME_ENABLED=false` 停止创建 Free Time 回合，不再为它调用模型或工具；
不影响聊天、Attention、定时提醒和记忆维护。重新开启后从正常间隔重新计时，
不补跑关闭期间的回合。`MAX_DAILY_PROACTIVE=0` 只限制主动消息送达，不等同于停用后台运行。
