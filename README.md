...
单主人使用，单进程加 SQLite；管理后台负责配置，日常直接聊天。
...
同协议换模型无需改代码。保存不调用模型；手动试生成会按服务商规则计费，失败不自动重试。
可预览生成结果或选择本地图片，再单独发送到主人微信。图片仅保留在当前页面和处理内存中，
没有持久图库；这还不是 Mochi 的生图 Skill，不会让她自动生成或发送图片。
...
| 自助更新 | 主人提出请求时检查官方正式版；先送达回复，再执行更新退出 |
...
Git 安装可以直接在聊天里让 Mochi 更新。它只在主人请求时检查 GitHub 官方正式 Release，不会每天轮询；更新保留 `.env`、`data/` 和个人扩展。
